...
import {loader} from "~/routes/stories+/$storyId+/_index.js";
...
export default function EditStoryRoute() {
    ...
    useEffect(() => {
        console.log(`## useEffect fetcher.data=${JSON.stringify(fetcher.data)} currentStory=${JSON.stringify(currentStory)}`)
        if (!currentStory && fetcher.state === "idle" && fetcher.data == null) {
            fetcher.load(`${path}?index`)
        }
    },  [currentStory, path, fetcher])

    console.log(`## edit fetcher.data=${JSON.stringify(fetcher.data)} currentStory=${JSON.stringify(currentStory)}`)
